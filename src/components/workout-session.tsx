...
export default function WorkoutSession({ workout, userId, onComplete }: WorkoutSessionProps) {
  ...
  useEffect(() => {
    if (sessionState === 'active' && userId && sessionId) {
      const autoSave = () => {
        workoutSessionService.updateWorkout(sessionId, {
  ...
  const startSession = async () => {
    ...
    try {
      ...
      const newSession = await workoutSessionService.createWorkout({
        user_id: userId,
        type: workout.name,
        plan_id: workout.id,
        duration_min: 0,
  ...
  const completeWorkout = async () => {
    ...
    if (sessionId) {
      try {
        await workoutSessionService.updateWorkout(sessionId, {
          duration_min: sessionData.duration_minutes,
          calories_est: sessionData.calories_estimated
        })
      } catch (error) {
