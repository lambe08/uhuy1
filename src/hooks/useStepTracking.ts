...
export function useStepTracking(userId: string | null, stepGoal: number = 10000) {
  ...
  const loadStepData = useCallback(async () => {
    ...
    try {
      ...
      const todayRecord = await stepRecordService.getStepsDaily(userId, today)

      // Get weekly steps
      const weeklySteps = await stepRecordService.getWeeklySteps(userId)

      // Get step history
      const history = await stepRecordService.getStepsHistory(userId, 30)
  ...
  const saveStepData = useCallback(async (steps: number) => {
    ...
    try {
      ...
      await stepRecordService.upsertStepsDaily({
        user_id: userId,
        date: today,
        steps,
